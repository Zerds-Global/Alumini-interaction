...
function Photo() {
  ...
  useEffect(() => {
    // Load only photo posts from Post API so we can use like/comment/share
    const token = localStorage.getItem("token");
    const collegeId = localStorage.getItem('college');
    const params = new URLSearchParams({ postType: 'photo' });
    if (collegeId) {
    ...
    fetch(`https://alumini-interaction.onrender.com/api/posts?${params.toString()}`, {
      ...
      .then((data) => {
        const posts = (Array.isArray(data) ? data : []).map((p) => ({
          ...
          image: p.image ? (p.image.startsWith('/uploads/') ? `https://alumini-interaction.onrender.com${p.image}` : p.image) : null,
          ...
          postedById: p.postedById ? (typeof p.postedById === 'object' ? p.postedById._id : p.postedById).toString() : null,
  ...
  const handleDelete = async (photo) => {
    ...
    if (photo.postedById && photo.postedById !== currentUserId) {
      alert('You can only delete your own posts.');
      return;
    }
    if (!window.confirm('Delete this photo post? This will remove the post and its photo.')) return;
    try {
      const token = localStorage.getItem("token");
      // Try deleting underlying photo first (ignore if missing)
      if (photo.referenceId) {
        try { 
          await fetch(`https://alumini-interaction.onrender.com/api/photo/${photo.referenceId}`, { 
            method: 'DELETE',
            headers: {
              "Authorization": `Bearer ${token}`,
              'Content-Type': 'application/json'
      ...
      const res = await fetch(`https://alumini-interaction.onrender.com/api/posts/${photo.id}`, {
        method: 'DELETE',
        headers: { 
          "Authorization": `Bearer ${token}`,
          'Content-Type': 'application/json' 
      ...
      alert('Photo post deleted');
    } catch (e) {
      console.error(e);
      alert('Failed to delete photo post');
    }
  };
  ...
  const handleImageChange = (e) => {
    setPhotoFormData({ ...photoFormData, image: e.target.files[0] });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (userRole !== 'alumni') {
      alert('Only alumni can upload photos.');
      return;
    }

    // Upload image to backend and create a Photo record
    const form = new FormData();
    form.append("heading", photoFormData.heading);
    form.append("description", photoFormData.description);
    form.append("image", photoFormData.image);

    const token = localStorage.getItem("token");
    fetch("http://127.0.0.1:5000/api/photo", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${token}`
      },
      ...
      .then(async (savedPhoto) => {
        // Create a Post referencing this photo
            const postRes = await fetch("https://alumini-interaction.onrender.com/api/posts", {
              ...
              body: JSON.stringify({
                heading: savedPhoto.heading,
                description: savedPhoto.description,
                image: savedPhoto.image, // path returned by backend
                postType: "photo",
        ...
        // Add returned post to UI feed
        const displayPhoto = {
          id: newPost._id,
          heading: newPost.heading,
          description: newPost.description,
          image: newPost.image ? (newPost.image.startsWith('/uploads/') ? `https://alumini-interaction.onrender.com${newPost.image}` : newPost.image) : null,
              referenceId: savedPhoto._id,
              postType: 'photo',
              ...
              >
                    ...
                    <div style={{ display: "flex", alignItems: "center", marginBottom: "15px" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div 
                          ...
                          onClick={() => {
                            if (photo.postedById) {
                              navigate(`/student/view-profile/${String(photo.postedById)}`);
                            }
                          }}
                          onMouseEnter={(e) => {
                            if (photo.postedById) {
                ...
                <Card.Body style={{ padding: "8px 25px 20px 25px" }}>
                  ...
                  <div style={{ 
                    ...
                    {(photo.postedById && photo.postedById === (localStorage.getItem('userId') || '')) && (
                      <Button
            ...
            <Form onSubmit={handleSubmit}>
              <Form.Group controlId="formHeading" className="mb-3">
                ...
                <Form.Control
                  type="text"
                  name="heading"
                  placeholder="e.g., Placement Ceremony 2025"
                  value={photoFormData.heading}
